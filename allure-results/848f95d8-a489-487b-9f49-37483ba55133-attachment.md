...
Call log:
  - waiting for getByRole('button', { name: /^India$/i })
...
> 29 |         await this.page.getByRole('button', { name: new RegExp(`^${paymentData.country}$`, 'i') }).click()
     |                                                                                                    ^ Error: locator.click: Test timeout of 30000ms exceeded.
  30 |         await this.placeOrder()
  31 |     }
  32 | 
  33 |     async placeOrder() {
  34 |         await this.placeOrderButton.click()
  35 |     }
  36 | }
```